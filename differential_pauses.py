...
def TPM_build(pauses,fwd,rev):
    RPK_total,return_dict = [],{}
    for key,value in pauses.iteritems():
        if value[0] == '+':
            RPK = sum(fwd[key-50:key+51])/0.101
            RPK_total.append(RPK)
            return_dict[key] = RPK
        else:
            RPK = sum(rev[key-50:key+51])/0.101
            RPK_total.append(RPK)
            return_dict[key] = RPK

    scaling = sum(RPK_total)/1000000

    for key,value in return_dict.iteritems():
        return_dict[key] = value/scaling

    return return_dict,scaling
...
def pause_rank(pauses,floor,scaling_WT,scaling_Mutant,fwd_WT,rev_WT,fwd_mutant,rev_mutant,wig_WT,wig_Mutant):
    ranked = {}
        ...
        if key in pauses['Mutant'].keys():
            ...
            if Mutant_counts != 0:
                if np.sign(WT_Counts) == np.sign(Mutant_counts):
                    if strand == '+':
                        Mutant_TPM = (sum(fwd_mutant[pos-50:pos+51])/0.101)/scaling_Mutant
                        if Mutant_TPM > floor:
                        ...
                        Mutant_TPM = (sum(rev_mutant[pos-50:pos+51])/0.101)/scaling_Mutant
                        if Mutant_TPM > floor:
                            ranked[key] = [abs(WT_Counts),abs(Mutant_counts),abs(value[2]),abs(Mutant_TPM),abs(value[1]),abs(Mutant_counts/Mutant_TPM),np.log2(abs(value[1])/abs(Mutant_counts/Mutant_TPM))]
                else:
                    if strand == '+':
    ...
    for key,value in pauses['Mutant'].iteritems():
        if key not in ranked.keys():
            ...
            if WT_Counts != 0:
                if np.sign(WT_Counts) == np.sign(Mutant_counts):
                    if strand == '+':
                        WT_TPM = (sum(fwd_WT[pos-50:pos+51])/0.101)/scaling_WT
                        if WT_TPM > floor:
                        ...
                        WT_TPM = (sum(rev_WT[pos-50:pos+51])/0.101)/scaling_WT
                        if WT_TPM > floor:
                            ranked[key] = [abs(WT_Counts),abs(Mutant_counts),abs(WT_TPM),abs(value[2]),abs(WT_Counts/WT_TPM),abs(value[1]),np.log2(abs(WT_Counts/WT_TPM)/abs(value[1]))]
                else:
                    if strand == '+':
...
def main():
    ...
    TPM_WT,scaling_WT = TPM_build(pauses_WT,WT_cov_fwd,WT_cov_rev)
    TPM_Mutant,scaling_Mutant = TPM_build(pauses_Mutant,Mutant_cov_fwd,Mutant_cov_rev)
    ...
    #rank all pauses
    ranked = pause_rank(final_pauses,floor,scaling_WT,scaling_Mutant,WT_cov_fwd,WT_cov_rev,Mutant_cov_fwd,Mutant_cov_rev,WT_wig,Mutant_wig)
