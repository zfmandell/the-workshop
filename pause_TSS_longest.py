...
def TU_build(TSS,TTS):
    ...
    TU_plus_temp,TU_minus_temp = {},{}
    TU_plus_final,TU_minus_final = {},{}
    ...
    for key,value in TU_plus.iteritems():
        if value not in TU_plus_temp.keys():
            TU_plus_temp[value] = key
        else:
            if key < TU_plus_temp[value]:
                TU_plus_temp[value] = key

    for key,value in TU_minus.iteritems():
            if value not in TU_minus_temp.keys():
                TU_minus_temp[value] = key
            else:
                if key > TU_minus_temp[value]:
                    TU_minus_temp[value] = key

    for key,value in TU_plus_temp.iteritems():
        TU_plus_final[value] = key
    for key,value in TU_minus_temp.iteritems():
        TU_minus_final[value] = key

    return TU_plus_final,TU_minus_final
...
placed_pauses = pause_place(pauses,TU_plus,TU_minus)
